...
Interface cho account client, cung cấp các phương thức truy xuất thông tin tài khoản, số dư và cấu hình giao dịch.

| Tên (Name) | Loại (Type) | Phân loại (Kind) | Mô tả (Description) |
|---|---|---|---|
| `GetBalanceUsdtAsync` | Task&lt;[`ApiResult`](./models.md#apiresult)&lt;[`AccountBalance`](./models.md#accountbalance)&gt;&gt; | Method | Lấy số dư tài khoản USDT mới nhất từ sàn giao dịch. |
| `WalletBalance` | `decimal` | Property | Tổng số dư ví thực tế (không gồm PnL chưa chốt). |
| `AvailableBalance` | `decimal` | Property | Số dư khả dụng để mở vị thế mới. |
| `Equity` | `decimal` | Property | Vốn tài khoản (Wallet Balance + Unrealized PnL). |
| `UnrealizedPnL` | `decimal` | Property | Tổng lợi nhuận/thua lỗ chưa chốt của các vị thế đang mở. |
| `InitialMargin` | `decimal` | Property | Tổng ký quỹ ban đầu đang sử dụng. |
| `MarginRatio` | `decimal` | Property | Tỷ lệ ký quỹ hiện tại của tài khoản. |
| `IsHedgeMode` | `bool` | Property | Kiểm tra tài khoản có đang ở chế độ Hedge Mode hay không. |
| `SetInitialLeverageAsync` | Task&lt;`bool`&gt; | Method | Cài đặt mức đòn bẩy cho mã giao dịch chỉ định. |
| `GetLeverage` | `decimal` | Method | Lấy mức đòn bẩy đã cấu hình cho mã giao dịch. |
| `SetHedgeModeAsync` | Task&lt;(`bool` Success, `string`? Error)&gt; | Method | Cài đặt chế độ vị thế (Hedge Mode vs Netting Mode). |
| `GetFeeLevelAsync` | Task&lt;[`ApiResult`](./models.md#apiresult)&lt;[`FeeVipLevel`](./enums.md#feeviplevel)&gt;&gt; | Method | Lấy cấp độ phí VIP hiện tại của tài khoản. |
...
Ghi chu tuong thich: host tu dong doc version `Pt.Okx.Sdk` ma plugin tham chieu luc compile de kiem tra tuong thich.
